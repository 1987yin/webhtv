...
签名覆盖规范化后的全部信封字段（`signature.value` 除外）和原始 v2 payload 摘要。客户端需要校验 HTTPS、签名、keyId、revision 单调性、算法版本、包大小、规则数、单规则帧数和过期时间；验证成功后再把 payload 交给现有严格 codec。写入采用临时文件加原子替换，并保留上一份可回滚快照。规则包下载地址不能由普通用户输入，避免把 `RuleLoader` 变成任意 HTTP 请求入口。
