...
| 23 | `C4` | 通用/上游应用合并 | 合并 `fish2018/webhtv:main` 的 APK 更新、局域网推送、MPV 字幕与阅读器修复 | **本地实施与验证完成，待原子提交/tag**：保留当前 `dev2` 文档、评估台账和本地播放器契约；不采用上游的文档清理删除；OCI 发布改为失败即阻断并固定 setup-oras 完整 commit | [C4-main-upstream-merge.md](C4-main-upstream-merge.md) |
