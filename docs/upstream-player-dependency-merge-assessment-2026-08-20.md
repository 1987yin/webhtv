...
| 31 | `C10` | 通用/播放器供应链 | 播放器 AAR、Maven sidecar、lock、MPV native override 和构建输入以上游为准 | **已验证**：选定发布输入对齐 `fish2018/webhtv:main@ec478b0b697422a7785171c7b51a35b7a526564e`；双 ABI MPV ELF 门禁与 Mobile/Leanback Arm64 Java 编译通过，不改 Java 播放行为或已打包播放器 `.so` | [C10-binary-upstream-alignment.md](C10-binary-upstream-alignment.md) |
