...
- 历史记录持久化已绑定 TMDB 剧集的标准季号和集号；续播匹配优先使用该标准位置，没有绑定数据的旧历史才回退 `Episode.getNumber()` 所复用的现有集数提取逻辑，不新增标题解析规则；
...
1. **新设置与现有「历史记录跨源聚合」相互独立。**
   ...
   - 现有设置决定是否使用 TMDB 身份聚合同一作品，以及是否优先按 TMDB 续播。
...
6. **已绑定 TMDB 剧集的位置必须持久化。**`History` 增加 `tmdbSeasonNumber`、`tmdbEpisodeNumber`，Room 数据库升级到 40；旧记录以 `(0, 0)` 表示未绑定并继续使用现有集数提取逻辑。标准集号大于 `0` 时，季号 `0` 表示特别篇，负数才表示季未知。
...
5. 不为旧历史离线猜测或批量回填季号、集号；仅在播放流程已绑定 `TmdbEpisode` 时保存标准位置。
...
1. 双方都绑定 TMDB 剧集时，优先使用标准集号；双方季号均已知时同时约束季号，其中 `0` 是合法的特别篇季号，负数才表示未知；
...
`VideoActivity.checkHistory()` 中显式的跨 `cid` 续播请求优先于当前目标源自己的旧进度，随后映射目标集并应用播放状态。普通详情入口仍走现有 `History.findPlayback()`，避免全局功能扩散到所有播放入口。
...
12. 外部历史和跨 `cid` 聚合卡可正确删除；全局清空有明确确认。
13. TMDB 未配置或聚合设置关闭时，功能仍可使用标题/年份保守匹配。
14. AI 推荐、观影报告、同步、WebHome 和外部 Browse 的数据范围不被本设置隐式扩大。
15. mobile 与 leanback 单测和 Debug 构建通过。
