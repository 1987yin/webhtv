...
| `dedupeKey` | string | 读取、Webhook | 播放条目去重 key，由 `configKey`+`siteKey`+`vodId`+`vodName`+`flag`+`episodeName`+`episodeUrl` 经 SHA256 计算，**不包含 token，也不包含 `historyKey`**。`historyKey` 含设备本机 `cid`，跨设备必然不同，纳入会导致两端 `dedupeKey` 不一致；`siteKey`/`vodId` 已单独参与，去掉 `historyKey` 不损失身份信息 |
...
`cid` 不能作为跨端接口身份，且 `historyKey`（含 `cid`）也不得进入 `dedupeKey` 计算：A、B 两台设备各自分配 `cid`，导致同一视频在两端算出的 `dedupeKey` 不同，跨设备删除（B 删 A 上传的记录）会永远命中失败（`affected=0`）。`dedupeKey` 仅由 `configKey`/`siteKey`/`vodId`/`vodName`/`flag`/`episodeName`/`episodeUrl` 这些跨设备稳定字段构成。同一远端 URL 和 token 下，服务端仍必须按 `configKey` 区分不同点播接口；否则 A 客户端的 b 接口记录可能覆盖 B 客户端的 a 接口记录。
...
**前提：`dedupeKey` 必须跨设备稳定**。它只能由 `configKey`/`siteKey`/`vodId`/`vodName`/`flag`/`episodeName`/`episodeUrl` 这些跨设备稳定字段计算，**不得包含 `historyKey`**（其中嵌有设备本机 `cid`，A、B 各自分配，会导致两端 `dedupeKey` 不同，删除永远 `affected=0`）。
