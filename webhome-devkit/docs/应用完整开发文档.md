...
| `dedupeKey` | string | 读取、Webhook | 播放条目去重 key，由 `configKey`+`siteKey`+`vodId`+`vodName`+`flag`+`episodeName` 经 SHA256 计算，**不包含 token，也不包含 `historyKey`，也不包含 `episodeUrl`**。`historyKey` 含设备本机 `cid`，跨设备必然不同；`episodeUrl` 多为带签名/节点的挥发 URL，且只在"完整"字段预设中上报、基础/标准预设不上报，服务端会存空值，纳入会导致 B 设备拉取后重算的 `dedupeKey` 与 A 不一致、跨设备删除永远 `affected=0`。去掉二者不损失身份信息（同一 站点+影片+线路+集名 仍唯一） |
...
| `historyKey` | string | 读取、写入、清理、Webhook | 本地历史主键，通常为 `siteKey@@@vodId` 或 `siteKey@@@vodId@@@cid`。含设备本机 `cid`，**只能用于本机落库与清理，不得用于跨设备去重或身份匹配，也不能进入 `dedupeKey` 计算** |
...
`cid` 不能作为跨端接口身份，且 `historyKey`（含 `cid`）也不得进入 `dedupeKey` 计算：A、B 两台设备各自分配 `cid`，导致同一视频在两端算出的 `dedupeKey` 不同，跨设备删除（B 删 A 上传的记录）会永远命中失败（`affected=0`）。此外 `episodeUrl` 也不得进入 `dedupeKey`：它只在"完整"字段预设中上报，基础/标准预设不上报时服务端存空值，B 拉取后重算的 `dedupeKey` 必然与 A 不同。`dedupeKey` 仅由 `configKey`/`siteKey`/`vodId`/`vodName`/`flag`/`episodeName` 这些跨设备稳定且必上报的字段构成。同一远端 URL 和 token 下，服务端仍必须按 `configKey` 区分不同点播接口；否则 A 客户端的 b 接口记录可能覆盖 B 客户端的 a 接口记录。
...
```json
{
  ...
  "deleted": [
    "f405e773d7a344b95bf0e9d6152234b8f60b17f185539eed4f9075884d6369f1"
  ]
}
```
...
`deleted` 是可选字段，服务端软删除某条播放记录后返回其 `dedupeKey` 墓碑列表（见 13.4.6 跨设备删除同步）。旧版服务端不返回该字段时，App 退化为不传播删除，保持向后兼容；返回空数组表示本次没有需要清理的本地记录。墓碑仅返回与当前 `configKey` 匹配的条目（空 `configKey` 为兼容旧数据一并返回），服务端会清理 90 天前的墓碑以避免无限增长。
...
| `playback.ended` | 自然完播 |
| `playback.deleted` | 本地删除某条历史时上报，供服务端软删除对应 `dedupeKey` 并向其它设备传播删除（见 13.4.6）。与进度/完播事件共用同一 `dedupeKey`，可精准定位；服务端本就无此条目时影响 0 行，无害 |
...
跨设备删除同步解决"在 A 设备删掉某条观看记录后，B 设备下次拉取时又被服务端重新推回"的问题。它依赖 `dedupeKey` 在上传、删除、墓碑比对三处使用同一算法（见 13.4.1），由"向上传播删除"和"向下拉取墓碑"两段闭环组成。

**前提：`dedupeKey` 必须跨设备稳定**。它只能由 `configKey`/`siteKey`/`vodId`/`vodName`/`flag`/`episodeName` 这些跨设备稳定且必上报的字段计算，**不得包含 `historyKey`**（含设备本机 `cid`，A、B 各自分配会导致两端 `dedupeKey` 不同，删除永远 `affected=0`），也**不得包含 `episodeUrl`**（多为挥发 URL，且只在"完整"预设上报，基础/标准预设下服务端存空值，B 拉取后重算必然与 A 不一致）。

**向上传播（A 删除 → 服务端软删）**

1. A 设备在本地删除历史（`History.delete` / `History.delete()`）时，若开启上报，会触发 `PlaybackEventCollector.onDeleted`，发送 `playback.deleted` Webhook 事件（payload 与进度事件共用同一 `dedupeKey`）。
2. 服务端收到后用 `UPDATE ... SET deletedAt = ? WHERE dedupeKey = ?` 软删除对应条目，并返回 `affected`（本就无此条目时影响 0 行，无害）。记录不物理删除，而是保留 `deletedAt` 时间戳作为墓碑。

**向下拉取墓碑（服务端 → B 清理本地）**

1. B 设备发起 `GET /` 远端同步时，服务端在返回 `items` 的同时，额外返回 `deleted` 列表：所有 `deletedAt > 0` 且匹配当前 `configKey` 的 `dedupeKey` 墓碑（见 13.4.4）。
2. B 设备解析响应取出 `deleted` 列表，对**每条本地 `History`** 调用 `PlaybackRecord.dedupeKeyFor` 用同一公式反算其 `dedupeKey`，与墓碑集合比对；命中则删除本地副本。
3. 该匹配只认 `dedupeKey`、不认 `historyKey`/`cid`，因此无论本地记录是原生创建还是从远端拉取所得，都能被正确清理；且只删服务端已软删过的记录，不会误删纯本地记录。

**闭环示意**

```text
A 删除本地记录 ──playback.deleted──▶ 服务端软删 dedupeKey=K3（留墓碑）
                                          │
B 拉取 GET / ◀── items:[...] + deleted:[K3] ─┘
   │
   └─ 对每条本地 History 算 dedupeKey，命中 K3 → 删除本地副本
```

**向后兼容与边界**

- 旧版服务端不返回 `deleted` 字段：App 用 `deletedKeysFromJson` 解析为空列表，退化为不传播删除，行为同旧版。
- 墓碑生命周期：服务端清理 90 天前的墓碑（`deletedAt < now - 90d`），超过该期限的删除事件不再向下游传播；期间若某设备长时间未同步，可能错过该墓碑而残留一条已删记录，下次该设备重新上报进度时又会被服务端软删并重新传播。
- 服务端删除只认 App 发来的 `dedupeKey`，不二次用 `vodId`/`siteKey` 等字段推导身份，因此要求 App 端 `dedupeKey` 公式稳定；若 App 升级改变了公式，旧服务端上用老公式生成的行需重置或重新上报覆盖。
...
- “观影记录同步”当前用于管理用户本地 Webhook 端点和当前播放只读 API。界面分为“开关”“远端同步”“Webhook 上报”三块：顶层只显示总开关、本机 API 修改开关和状态摘要；远端同步源列表与 Webhook 端点列表点击后进入二级界面，新增/编辑都在独立表单中完成。Webhook 上报时机固定为进度更新、退出/暂停/切集前最终进度和自然完播，不再让用户选择；此外在本地删除观看历史时会额外上报 `playback.deleted` 事件以向其它设备传播删除（见 13.4.6）。字段预设为“基础/标准/完整/自定义”，自定义字段使用带说明的多选列表。Webhook 和远端同步 token 均由用户服务端统一提供，App 只负责保存并随请求发送。协议字段、远端同步源和本机修改 API 规范见 13.4。
